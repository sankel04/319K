...
uint32_t killedEnemys = 0;
uint32_t delayTime = 0;

typedef enum {English, Spanish} Language_t;
Language_t myLanguage=English;
typedef enum {HELLO, GOODBYE, LANGUAGE, WELCOME,INSTRUCTION1, INSTRUCTION2,OutroLose, OutroWin,PAUSE,RESTART} phrase_t;
...
const char OutroLose_English[] ="GAME OVER\n Nice Try \n Earthling";
const char OutroLose_Spanish[] ="Juego terminado\n Buen intento \n terricola";
const char OutroWin_English[] ="Great Job!\n Earthling";
const char OutroWin_Spanish[] ="Buen trabajo!\n terricola";
const char Pause_English[] ="pause";
const char Pause_Spanish[] ="pausa";
const char Restart_English[] ="< restart";
const char Restart_Spanish[] ="< reanudar";
...
const char *Phrases[10][2]={
  ...
  {Language_English,Language_Spanish},
   ...
   {OutroLose_English,OutroLose_Spanish},
  {OutroWin_English,OutroWin_Spanish},
...
void Move(void){
  ...
  for(i=0; i<6; i++){
      if(Enemys[i].life == alive){
         ...
         for(uint8_t cnt = 0; cnt<playerCNT;cnt++){ // iterate through all player bullets and check against all alive enemies
             if(playerBullets[cnt].life == alive){
                 if((playerBullets[cnt].y+2) <= (Enemys[i].y+2)){ // if player bullet made it to enemy y
                     if(((playerBullets[cnt].x+2) >= (Enemys[i].x + 2)) && ((playerBullets[cnt].x+2) <= (Enemys[i].x +14))){
                         Enemys[i].life = dead;
                         Sound_Killed();
                         playerBullets[cnt].life = dead;
                         killedEnemys++;
  ...
  for(uint8_t i=0;i<20;i++){ // iterate through all bullets and move them down / up
      ...
      if(Bullets[i].life == alive){
          if((Bullets[i].y >= 142) && (Bullets[i].y <= 148)){ // checks if bullet at bottom
...
void genBullets(void){
    ...
    if(Enemys[ran].life == alive){
        Bullets[bulletIndex].x = Enemys[ran].x + 7;
        Bullets[bulletIndex].y = Enemys[ran].y + 7;
...
void genPlayerBullet(void){
    Sound_Shoot();
    playerCNT++;
    playerBullets[playerBulletIndex].x = player.x + 8;
    playerBullets[playerBulletIndex].y = player.y - 5;
...
// game engine goes here
    if(Switch_In() == 1){
      //  Clock_Delay1ms(100);
        ...
        ST7735_OutString((char *)Phrases[PAUSE][myLanguage]);
        ST7735_OutChar(13);
        ST7735_OutString((char *)Phrases[RESTART][myLanguage]);
        while(1){
            if(Switch_In() == 1){
    ...
    if(player.life == dead){
        ...
        ST7735_OutString((char *)Phrases[OutroLose][myLanguage]);
        ST7735_SetCursor(2, 4);
        ST7735_OutUDec(killedEnemys);
        while(1){
            Clock_Delay1ms(750);
    ...
    if(killedEnemys == 6){
               ...
               ST7735_SetCursor(1, 1);
               ST7735_FillScreen(0x0000);   // set screen to black
               ST7735_OutString((char *)Phrases[OutroWin][myLanguage]);
               ST7735_SetCursor(2,4);
               ST7735_OutUDec(killedEnemys);
               while(1){
                   Clock_Delay1ms(750);
...
void SysTick_Handler(void){ // called at 11 kHz
  DAC5_Out(dacVal()); // output one value to DAC if a sound is active
}
